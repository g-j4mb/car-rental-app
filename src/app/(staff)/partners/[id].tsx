...
export default function EditPartnerScreen() {
  ...
  const { control, handleSubmit, reset, formState: { errors } } = useForm<PartnerInput>({
    ...
    defaultValues: { name: '', phone: '', commission_rate: 20, notes: '' },
  });
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <TouchableOpacity
        style={[styles.submitBtn, updatePartner.isPending && styles.submitBtnDisabled]}
        onPress={handleSubmit(onSubmit)}
        disabled={updatePartner.isPending}
      >
        {updatePartner.isPending && <ActivityIndicator color="#fff" style={{ marginRight: 8 }} />}
        <Text style={styles.submitText}>
          {updatePartner.isPending ? t('common.saving') : t('partners.save_changes')}
        </Text>
      </TouchableOpacity>
    </ScrollView>
  );
